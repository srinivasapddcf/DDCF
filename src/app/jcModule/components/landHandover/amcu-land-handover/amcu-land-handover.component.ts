...
@Component({
  selector: 'app-amcu-land-handover',
  templateUrl: './amcu-land-handover.component.html',
  styleUrls: ['./amcu-land-handover.component.css'],
})
export class AmcuLandHandoverComponent implements OnInit {
  maxDate: Date;
  input = '';
  date: any;
  ...
  ngOnInit(): void {
    ...
    this.LandAllocateData.milkPouringVillageId = decString.milkPouringVillageId ?? '';

    if (this.LandAllocateData.districtId === '') {
      this.router.navigate(['/jcModule/AmcuLandAllocation']);
      ...
      this.router.navigate(['/jcModule/AmcuLandAllocation']);
    } else if (this.LandAllocateData.milkPouringVillageId === '') {
      this.router.navigate(['/jcModule/AmcuLandAllocation']);
    }
  ...
  async loadPossessionTaken(): Promise<void> {
    try {
      const req = {
        ...
        milkPouringVillageId: this.LandAllocateData.milkPouringVillageId
      };
      this.spinner.show();
      const response = await this.jcAPI.amcuPossessionTakenDetails(req);
      if (response.success) {
  ...
  async btnSubmit(): Promise<void> {
    try {
      if (this.validate()) {
        ...
        this.spinner.show();
        const response = await this.jcAPI.amcuLandHandOverSub(
