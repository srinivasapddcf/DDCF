...
export class CheckListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.divSbmt = false;this.tremarks=false;
    this.loadRbkList();
  }
  ...
  async loadRbkList(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const res = await this.dcoAPI.dcorbkListByDivisionId(req);
      this.spinner.hide();
      this.rbkList = [];
      if (res.success) {
  ...
  bntenablecheck(): void {
    if (this.checkList.length == this.answrdcheckList.length) {
      this.divSbmt = true;//this.tremarks=false;
    } else {
      this.divSbmt = false; //this.tremarks=true;
    }
  }
