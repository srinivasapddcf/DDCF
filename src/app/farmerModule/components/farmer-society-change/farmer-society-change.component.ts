...
export class FarmerSocietyChangeComponent  implements OnInit, OnDestroy, AfterViewInit
{
  oldRbkList = [];
  oldVillageList = [];
  newRbkList = [];
  ...
  async loadOldRBKList(): Promise<void> {
    try {   debugger;
      const req = {
        type:"12",
        UNIQUE_ID: this.session.uniqueId,//"1236158", 
      };
      this.spinner.show();
  ...
  async onOldRbkChange( ): Promise<void> {
    try { 
      ...
      const req = {       
        type:"13",
        rbk_id:this.farmerSocietyData.oldRbkId
      }
      this.spinner.show();
     // this.oldVillageList = [];
      const response = await this.mcuAPI.TransactionsByUnionidGet(req);

      //this.farmerid = response.result[0].FARMER_CODE;
      this.spinner.hide();
      if (response.success) {
  ...
  async onOldVillageChange(): Promise<void> {
    try {
      ...
      const req = {
        ...
        village_id: this.farmerSocietyData.oldVillageId,
        //input1:this.farmerid
      };
      this.spinner.show();
      const response = await this.mcuAPI.TransactionsByUnionidGet(req);
      this.spinner.hide();
...
debugger;
      ...
      this.spinner.show();
      const response = await this.mcuAPI.farmerSocietyChangeDAVFWEARequestSub(
  ...
  btnSocietyChangeRequests(): void {   debugger;
    this.router.navigate(['/farmer/SocietyChangeRequests']);
  }
