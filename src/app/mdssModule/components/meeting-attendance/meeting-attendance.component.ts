  ...
  implements  OnInit, OnDestroy, AfterViewInit
{
  ...
  async loadRBKList(): Promise<void> {
    try {
      const req = {
        rbkId: this.session.rbkGroupId,meetingId:'100'
      };
      debugger;
      this.spinner.show();
      //const response = await this.attendanceAPI.rbkListByMentorId(req);
      const response = await this.attendanceAPI.farmerDetailsByMeetingId(req);
             if (response.success) {
  ...
  async onRbkChange(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const response = await this.attendanceAPI.meetingDetailsByRbkId(req);
  ...
  async onMeetingChange(): Promise<void> {
    try {
      ...
      debugger;
      if (response.success) {
        if(response.result[0].BSTATUS=="1")
        {
          this.MeetingStatus=false;
        }
        if(response.result[0].BSTATUS=="0")
        {
