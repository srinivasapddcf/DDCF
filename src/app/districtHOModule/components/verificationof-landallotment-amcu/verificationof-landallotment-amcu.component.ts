...
@Component({
  selector: 'app-verificationof-landallotment-amcu',
  templateUrl: './verificationof-landallotment-amcu.component.html',
  styleUrls: ['./verificationof-landallotment-amcu.component.css']
})
export class VerificationofLandallotmentAMCUComponent implements OnInit,OnDestroy, AfterViewInit {
  districtId = '';
  districtList = [];
...
if( this.MANDAL_CODE===''||  this.MANDAL_CODE===undefined || this.MANDAL_CODE==='0' )
{
  this.toast.info("Please Select Mandal"); return;
    ...
    const res = await this.districtHOAPI.amcuLandAllotmentMandalReport(req);
      if (res.success) {
        this.excelData = [];
        ...
        for (let i = 0; i < this.mandalLevelDetails.length; i++) {
          let singleRow = {
            S_NO: i + 1,
            RSKNAME: this.mandalLevelDetails[i].RBK_NAME,
      ...
      if( this.MANDAL_CODE===''||  this.MANDAL_CODE===undefined || this.MANDAL_CODE==='0' )
{
  this.toast.info("Please Select Mandal"); return;
      ...
      if (res.success) {
        this.toast.info(res.message);
        this.loadReport();
      //  this.router.navigate(['/districtHO/VerificationofLandallotmentAMCU']);
      //  this.spinner.hide();
      }
     // this.rerender();
    }
