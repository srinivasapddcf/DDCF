...
export class BmcuEquipmentReportComponent implements OnInit, OnDestroy, AfterViewInit {
  BMCUEquipmentList: any[] = []; 
  excelData: any[] = [];  Districtlist: any[] = [];  Bmculist: any[] = []; 
  BMCUID:any;
  Districtid:any;
          ...
          if( res.success ){
            ...
            if ( res.result.length != 0 ) {
              ...
              for (let i = 0; i < this.BMCUEquipmentList.length; i++) {
                // this.reportTotals.MANDAL += parseInt(this.BMCUEquipmentList[i].MANDAL);                
                // this.reportTotals.RBK += parseInt(  this.BMCUEquipmentList[i].RBK );                
                // this.reportTotals.BMCU += parseInt( this.BMCUEquipmentList[i].BMCU );                 
                // this.reportTotals.EQUIPMENT += parseInt( this.BMCUEquipmentList[i].EQUIPMENT );
  ...
  try {debugger;
    this.Bmculist =[];
      ...
      if (res.success) {
          ...
          this.toast.info("Details Not Found");//res.message);
      }
      this.spinner.hide();
  } catch (error) {
      this.spinner.hide();
